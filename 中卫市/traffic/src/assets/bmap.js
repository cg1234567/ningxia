...
const Mixin = {
  ...
  methods: {
    ...
    initMap() {
      ...
      this.mapObj.centerAndZoom(new window.BMapGL.Point(105.221638,37.501017), 16); // 初始化地图,设置中心点坐标和地图级别
    ...
    setshequ(marks, cb, cb1) {
      ...
      marks.forEach((item, index) => {
        ...
        let iconns = `<div class="selqy">
                        <div style="display:flex;flex-direction:column;align-items:center;">
                          <img src=${require('@/assets/image/sg1.png')} >
        ...
        let icons = `<div class="selqy">
                        <div style="display:flex;flex-direction:column;align-items:center;">
                          <img src=${require('@/assets/image/sg2.png')} >
                          <span style="color:#fff">${item.name}</span>
                        </div>
                        <div class="camera" onclick="openvideo()"></div>
                     </div>`
        let icon = item.type==1 ? icons : iconns
